...
def run_test(dataset, hps, logdir, mode):
    with tf.variable_scope("model"):
        hps.num_sampled = 0  # Always using full softmax at evaluation.
        hps.keep_prob = 1.0
        model = LM(hps, mode, "/cpu:0")
        ...
        loss_nom = 0.0
        loss_den = 0.0
        print('eval_type: ', hps.eval_type)
        print('num_steps: ',hps.num_steps)
        result_file = open(logdir+'/'+mode+'_'+hps.eval_type,mode='w')
        with sess.as_default():
            print('Load Checkpoint: ',ckpt_loader.load_checkpoint())
            ...
            for i, (x, y, w) in enumerate(data_iterator):
                print('iterate: ',i)
                if mode=='generate':
                    generated = sess.run(model.generated, {model.x: x})
                    for i in range(hps.num_gpus):
                        for j in range(hps.batch_size):
                            result_file.write(' '.join([dataset.id_to_word(generated[i][k][j]) for k in range(hps.num_g)])+"\n")
                else:
                    loss = sess.run(model.loss, {model.x: x, model.y: y, model.w: w})
                    print("shape: ", len(loss[0][0]))
                    if hps.eval_type=='normal':
                        ...
                        for i in range(hps.num_gpus):
                            for j in range(hps.batch_size):
                                result_file.write(' '.join([str(k) for k in loss[i][j]])+"\n")
            if hps.eval_type=='normal':
